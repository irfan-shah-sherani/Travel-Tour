...
app.get("/download-pdf", async (req, res) => {
  ...
  db.query("SELECT * FROM TravalRecord WHERE qr_token = ?", [token], async (err, results) => {
    ...
    const baseUrl = process.env.BASE_URL || "http://localhost:3000";
    const url = `${baseUrl}/pdf.html?token=${token}`; // this will render your EJS with QR

    const file = { url: url }; // Use rendered HTML page URL
    ...
    try {
      ...
      res.send(pdfBuffer); // ⬇️ triggers direct download in browser
    } catch (err) {
      console.error("PDF generation error:", err);
      res.status(500).send("Failed to generate PDF");
    }
